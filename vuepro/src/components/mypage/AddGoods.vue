        ...
        methods: {
            ...
            doAddGoods(){
                ...
                if(this.form.商品名称==''||this.form.商品编号==''||this.form.商品价格==''||this.form.商品单位==''||this.form.商品品牌=='')
                {
                    ...
                    this.$axios.post('http://localhost:8080/WaterWarm1.0/AddGoods',jsonparam)
					.then(res=> {
						let addres=res.data;
						console.log(addres);
            ...
            getMaxid(){
                this.$axios.get('http://localhost:8080/WaterWarm1.0/GetMaxGoodsId')
                .then(res=> {
                    let result = res.data;
                    console.log(result);
                    if(result.code=='200'){
                        this.form.商品编号=result.maxid+1;
                    }else{
                        this.$message.error('网络可能存在异常，请稍后添加商品');
                    }
